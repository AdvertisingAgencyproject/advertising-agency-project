        ...
        <div className="app">
            <Router>
                ...
                <Routes>
                    ...
                    { authStore.isManager() && <Route exact path="/manager" element={<ManagerPage/>}/> }
                    { authStore.isAdmin() && <Route exact path="/admin" element={<AdminPage/>}/> }
                    { (authStore.isAdmin() || authStore.isManager()) && <Route exact path="/admin/favors" element={<FavorsAdmin/>}/> }
                    { (authStore.isAdmin() || authStore.isManager()) && <Route exact path="/admin/products" element={<ProductsAdmin/>}/> }
